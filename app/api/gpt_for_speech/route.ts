...
export async function POST(request: Request) {
  ...
  if (conversationHistory.length === 0) {
    conversationHistory.push({
      role: "system",
      content: `あなたはユーザーの日記を主観的にとらえ、内容に沿って過去のユーザという人格で主観的に演じてください。「こんにちは、私は過去のあなたです。」という挨拶から始まり、ユーザーに過去の自分を会話しているような体験を提供してください。ただし全ての質問に対して単なる日記の書き写しは厳禁です。日記情報は[日付,内容)]の形式です。日記: 質問: ${prompt}`,
    });
  }
